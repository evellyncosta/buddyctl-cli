...
class ChatClient:
    ...
    def chat_stream(self, agent_id: str, prompt: str, on_message: Callable[[str], None]) -> None:
        ...
        with httpx.Client(timeout=self.timeout) as client:
            with client.stream("POST", url, json=request.__dict__, headers=headers) as response:
                if response.status_code != 200:
                    ...
                    try:
                        ...
                        error_content = (
                            f"Unable to read error details (status {response.status_code})"
                        )
                    ...
                    if response.status_code == 403:
                        ...
                        error_msg = (
                            "Unauthorized - Authentication failed. Try refreshing your token"
                        )
    ...
    def _process_sse_stream(
        self, response: httpx.Response, on_message: Callable[[str], None]
    ) -> None:
        """
