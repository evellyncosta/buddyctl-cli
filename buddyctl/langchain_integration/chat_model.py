...
class StackSpotChatModel(BaseChatModel):
    ...
    stackspot_knowledge: bool = Field(
        default=False, 
        description="Use StackSpot Knowledge Sources"
    )
    return_ks_in_response: bool = Field(
        default=True,
        description="Return Knowledge Sources in response"
    )
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            response = self._chat_client.chat_non_stream(
                self.agent_id,
                user_prompt
            )
